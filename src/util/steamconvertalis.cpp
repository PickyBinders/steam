...
static void printSeqBasedOnAln(std::string &out, const char *seq, unsigned int offset,
...
static std::map<unsigned int, unsigned int> readKeyToSet(const std::string& file) {
...
static std::map<unsigned int, std::string> readSetToSource(const std::string& file) {
...
int steamconvertalis(int argc, const char **argv, const Command &command) {
    ...
    bool needTeaDB = false;

    // Pre-process outfmt to handle TEA-specific columns before passing to base parser
    // Replace tea-specific tokens with "empty" so the base parser doesn't error,
    // then record positions and codes for our custom handling
    std::string modifiedOutfmt = par.outfmt;
    std::vector<std::pair<size_t, int>> teaColumns; // (position_in_outcodes, tea_code)
    {
        std::vector<std::string> tokens = Util::split(par.outfmt, ",");
        std::string rebuilt;
        for (size_t i = 0; i < tokens.size(); i++) {
            int teaCode = -1;
            if (tokens[i] == "tfident") { teaCode = LocalParameters::OUTFMT_TFIDENT; needBacktrace = true; needTeaDB = true; }
            else if (tokens[i] == "tpident") { teaCode = LocalParameters::OUTFMT_TPIDENT; needBacktrace = true; needTeaDB = true; }
            else if (tokens[i] == "qteaseq") { teaCode = LocalParameters::OUTFMT_QTEASEQ; needTeaDB = true; }
            else if (tokens[i] == "tteaseq") { teaCode = LocalParameters::OUTFMT_TTEASEQ; needTeaDB = true; }
            else if (tokens[i] == "qteaaln") { teaCode = LocalParameters::OUTFMT_QTEAALN; needBacktrace = true; needTeaDB = true; }
            else if (tokens[i] == "tteaaln") { teaCode = LocalParameters::OUTFMT_TTEAALN; needBacktrace = true; needTeaDB = true; }

            if (teaCode >= 0) {
                teaColumns.push_back(std::make_pair(i, teaCode));
                if (i > 0) rebuilt += ",";
                rebuilt += "empty"; // placeholder for base parser
            } else {
                if (i > 0) rebuilt += ",";
                rebuilt += tokens[i];
            }
        }
        modifiedOutfmt = rebuilt;
    }

    std::vector<int> outcodes = Parameters::getOutputFormat(format, modifiedOutfmt, needSequenceDB, needBacktrace, needFullHeaders,
                                                             needLookup, needSource, needTaxonomyMapping, needTaxonomy);
    // Patch in TEA codes at the recorded positions
    for (size_t i = 0; i < teaColumns.size(); i++) {
    ...
    if (format == Parameters::FORMAT_ALIGNMENT_SAM) {
        ...
        Debug(Debug::ERROR) << "HTML format not supported in steam convertalis\n";
        EXIT(EXIT_FAILURE);
                            ...
                            for(size_t i = 0; i < outcodes.size(); i++) {
                                switch (outcodes[i]) {
                                    ...
                                    case Parameters::OUTFMT_BITS:
                                        // Both raw and bits output the raw alignment score
                                        // (no bit score conversion — Gumbel doesn't apply to TEA search)
                                    ...
                                    case LocalParameters::OUTFMT_TFIDENT:
                                    case LocalParameters::OUTFMT_TPIDENT: {
                                        // Compute TEA identity from backtrace
                                        // Main DB sequences are TEA sequences
                                        float teaFident = 0;
                                        if (res.backtrace.empty() == false) {
                                            int qPos = res.qStartPos;
                                            int tPos = res.dbStartPos;
                                            ...
                                            for (size_t pos = 0; pos < unpackedBt.size(); pos++) {
                                                switch (unpackedBt[pos]) {
                                                    case 'M':
                                                        teaIdentical += (querySeqData[qPos] == targetSeqData[tPos]);
                                                        teaMatchCount++;
                                                        qPos++;
                                                        tPos++;
                                                        break;
                                            ...
                                            teaFident = (teaMatchCount > 0) ? static_cast<float>(teaIdentical) / static_cast<float>(alnLen) : 0;
                                        }
                                        if (outcodes[i] == LocalParameters::OUTFMT_TPIDENT) {
                                            result.append(SSTR(teaFident * 100));
                                        } else {
                                            result.append(SSTR(teaFident));
                                        }
                                        break;
                                    }
                                    case LocalParameters::OUTFMT_QTEASEQ:
                                        result.append(querySeqData, res.qLen);
                                        break;
                                    case LocalParameters::OUTFMT_TTEASEQ:
                                        result.append(targetSeqData, res.dbLen);
                                        break;
                                    case LocalParameters::OUTFMT_QTEAALN: {
                                        if (res.backtrace.empty() == false) {
                                            int qPos = res.qStartPos;
                                            std::string unpackedBt = Matcher::uncompressAlignment(res.backtrace);
                                            for (size_t pos = 0; pos < unpackedBt.size(); pos++) {
                                                if (unpackedBt[pos] == 'M' || unpackedBt[pos] == 'I') {
                                                    result.push_back(querySeqData[qPos]);
                                                    qPos++;
                                                } else {
                                                    result.push_back('-');
                                                }
                                    ...
                                    case LocalParameters::OUTFMT_TTEAALN: {
                                        if (res.backtrace.empty() == false) {
                                            int tPos = res.dbStartPos;
                                            std::string unpackedBt = Matcher::uncompressAlignment(res.backtrace);
                                            for (size_t pos = 0; pos < unpackedBt.size(); pos++) {
                                                if (unpackedBt[pos] == 'M' || unpackedBt[pos] == 'D') {
                                                    result.push_back(targetSeqData[tPos]);
                                                    tPos++;
                                                } else {
                                                    result.push_back('-');
                                                }
