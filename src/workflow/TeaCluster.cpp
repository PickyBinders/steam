...
#include "teacluster.sh.h"

#include <cassert>

void setTeaClusterWorkflowDefaults(LocalParameters *p) {
    p->spacedKmer = true;
    p->covThr = 0.8;
    p->evalThr = 0.01;
...
float setAutomaticTeaClusterThreshold(float seqId) {
    float sens;
    if (seqId <= 0.3) {
        sens = 8;
    } else if (seqId > 0.8) {
...
void setTeaClusterAutomagicParameters(LocalParameters &par) {
    ...
    if (par.PARAM_S.wasSet == false) {
        par.sensitivity = setAutomaticTeaClusterThreshold(par.seqIdThr);
...
int teacluster(int argc, const char **argv, const Command &command) {
    ...
    setTeaClusterWorkflowDefaults(&par);
    ...
    setTeaClusterAutomagicParameters(par);
