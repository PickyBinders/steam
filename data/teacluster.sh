...
if [ "${RUN_LINCLUST}" = "1" ]; then
  ...
  # 1. Finding exact k-mer matches (on TEA sequences)
...
if [ "${RUN_ITERATIVE}" = "1" ]; then
  ...
  while [ "$STEP" -lt "$STEPS" ]; do
      ...
      if notExists "${TMP_PATH}/aln_step$STEP.dbtype"; then
          # shellcheck disable=SC2086
          $RUNNER "$MMSEQS" align "${INPUT}" "${INPUT}" "${TMP_PATH}/pref_step$STEP" "${TMP_PATH}/aln_step$STEP" ${ALIGNMENT_PAR} \
...
if [ -n "$REASSIGN" ]; then
    ...
    # align all members to cluster centroids
    if notExists "${TMP_PATH}/aln.dbtype"; then
        # shellcheck disable=SC2086
        $RUNNER "$MMSEQS" align "${SOURCE}" "${SOURCE}" "${TMP_PATH}/clu" "${TMP_PATH}/aln" ${ALIGNMENT_PAR} \
                || fail "Alignment step $STEP died"
    fi
    # create file of cluster members that do not align based on given criteria
    ...
    if [ ! -s "${TMP_PATH}/clu_not_accepted_swap.index" ]; then
        ...
        # create file of cluster members that do align based on given criteria
        ...
        # create sequence database of wrongly assigned members
        ...
        # build seed (centroid) sequence database
        ...
        if notExists "${TMP_PATH}/seq_wrong_assigned_pref.dbtype"; then
            if notExists "${TMP_PATH}/seq_seeds.merged.dbtype"; then
                ...
                # also merge _aa companion DBs
                if exists "${TMP_PATH}/seq_wrong_assigned_aa.dbtype"; then
                    buildMergedDb "${TMP_PATH}/seq_wrong_assigned_aa" "${TMP_PATH}/seq_seeds_aa" "${TMP_PATH}/seq_seeds.merged_aa"
                fi
            fi
            # shellcheck disable=SC2086
        ...
        if notExists "${TMP_PATH}/seq_wrong_assigned_pref_swaped_aln.dbtype"; then
            # shellcheck disable=SC2086
            $RUNNER "$MMSEQS" align "${TMP_PATH}/seq_seeds.merged" "${TMP_PATH}/seq_wrong_assigned" \
